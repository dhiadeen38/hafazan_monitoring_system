<?php
  ...
  $query = "select password from admin where admin_id='$name' ";
  ...
  if ($data_pwd==$curr_pwd) {
    if ($new_pwd==$new_pwd2) {
      $changepwd_sql="UPDATE `admin` SET `password` = '$new_pwd' WHERE `admin`.`admin_id` = '$name'";
      ...
      if($changepwd_sql){
        $_SESSION['message']='<div class="alert alert-success alert-dismissable">
  			...
  			header("location:changepwdadmin.php ");
