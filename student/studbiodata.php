...
<body>
	...
	$sql="SELECT student.stud_name,student.gender, student.stud_id, student.stud_status, programme.programme_name,  student.sponsor, batch.batch_name, student.ic_no, student.phone_no, student.email, student.profile_pic
	FROM student
	INNER JOIN programme
	ON student.programme_id=programme.programme_id
	INNER JOIN batch
	ON student.batch_id=batch.batch_id
	WHERE student.stud_id='$id' OR student.ic_no='$id'";
	$result=mysqli_query($con,$sql);

	$pic_sql="SELECT profile_pic
	FROM student
	WHERE stud_id='$id' OR ic_no='$id'";
	...
	<div id="wrapper">
		...
		<div id="sidebar-wrapper">
			<ul class="sidebar-nav">
				...
				<li><a style="background: yellow; color:#522184" href="studbiodata.php"><i class="fa fa-user"></i> Biodata</a></li>
					...
					<div class="card-deck col-lg-12">
					  ...
					  <div class="card ml-0">
							...
							<div class="card-body">
								...
								echo '<tr align="left">';
								echo '<th scope="row">'.'Student status'.'</th>';
								echo '<td>'.$row[3].'</th>';
								echo '</tr>';

								echo '<tr align="left">';
								echo '<th scope="row">'.'Programme'.'</th>';
								echo '<td>'.$row[4].'</th>';
								echo '</tr>';

								echo '<tr align="left">';
								echo '<th scope="row">'.'Sponsor'.'</th>';
								echo '<td>'.$row[5].'</th>';
								echo '</tr>';

								echo '<tr align="left">';
								echo '<th scope="row">'.'Batch'.'</th>';
								echo '<td>'.$row[6].'</th>';
								echo '</tr>';

								echo '<tr align="left">';
								echo '<th scope="row">'.'IC No'.'</th>';
								echo '<td>'.substr($row[7],0,6).'-'.substr($row[7],-6,-4).'-'.substr($row[7],8).'</th>';
								echo '</tr>';

								echo '<tr align="left">';
								echo '<th scope="row">'.'Phone No'.'</th>';
								echo '<td>'.substr($row[8],0,3).'-'.substr($row[8],3).'</th>';
								echo '</tr>';
											...
											<div class="modal-body">
												<form action="update_bio.php" method="post">
													...
													<div class="form-group row">
														...
														<div class="col-sm-9">
															<input type="text" name="phone_no" class="form-control" id="phone_no" placeholder="Phone" value="<?php echo $row[7]; ?>">
														</div>
													</div>
													...
													<div class="form-group row">
														...
														<div class="col-sm-9">
															<input type="text" name="email" class="form-control" id="email" placeholder="Email" value="<?php echo $row[8]; ?>">
														</div>
													</div>
