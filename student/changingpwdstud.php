<?php
  ...
  $query = "select password from student where student.stud_id='$name' ";
  ...
  if ($data_pwd==$curr_pwd) {
    if ($new_pwd==$new_pwd2) {
      ...
      $changepwd_sql="UPDATE `student` SET `password` = '$new_pwd' WHERE `student`.`stud_id` = '$name'";
      ...
      if($changepwd_sql){
        $_SESSION['message']='<div class="alert alert-success alert-dismissable">
  			...
  			header("location:changepwdstud.php ");
